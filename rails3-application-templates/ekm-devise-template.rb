...
if config['devise']
  config['addmodel'] = yes_wizard?("Would you like to add a model linked to the user class?") if true && true unless config.key?('addmodel') 
end
...
RUBY
  ...
  if recipes.include? 'devise'
    ...
    if recipes.include? 'addmodel'
      puts "\nEnter the name of the model: "
      model_name = STDIN.gets
      model_name = model_name.chomp!
      puts "Enter the fields and field types (we will put the \"user_id:integer\" in for you):"
      model_fields = STDIN.gets
      model_fields = model_fields.chomp!
      generate "scaffold #{model_name} #{model_fields} user_id:integer"
      # gsub_file 'app/models/book.rb', 'belongs_to :user'
      insert_into_file 'app/models/book.rb', "\n  belongs_to :user\n", :before => "\nend\n" # :after => "jquery_ujs\n"
      # plural_model = pluralize(model_name)
      insert_into_file 'app/models/user.rb', "\n  has_many :#{model_name}s\n", :before => "\nend\n" # :after => "jquery_ujs\n"
    end
